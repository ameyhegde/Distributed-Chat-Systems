...
void join_handler(char *ipaddr_new, char *port, char *uname) {
	...
	char joinMsg1[MESSAGE_SIZE];
		...
		for (map<string,vector<string> >::iterator it1=memberInfo.begin(); it1!=memberInfo.end(); ++it1) {
			char *ipPort = strcpy((char*)malloc( (it1->first).length()+1), (it1->first).c_str());
			char *ipVal = strtok (ipPort,":");
...
void *chat_send(void *myvar) {
	...
	while (true) {
			...
			cout<<t1.tv_usec - timeOfDay<<"time"<<endl;
			if((t1.tv_usec - timeOfDay) > 100 ){
			...
			//cout<<t.tv_usec - timeOfDay<<"time"<<endl;
			Messagecount++;	
			/*if (Messagecount< 1){
				cout<<"hey I'm here"<<endl;
				cout<<timeOfDay<<" time of day"<<endl;
				sleep(1);
			}*/
			strcpy(msg,"2-");
