		...
		switch(t) {
				...
				{	
					...
					if(current_seq == prev_seq + 1){
							...
							if (strcmp(ip_leader, ipaddress) == 0 && portNo == port_leader) {
  								printQueue.push(string(message));
  							}else {

  								string tempVar = senderNode.append( string(":").append( deCipher(act_msg) ) );
  								printQueue.push(tempVar);
